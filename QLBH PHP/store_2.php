        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $('#Submit').livequery('click', function () {
                    ...
                    $('#cart_wrapper').html('Tổng Tiền: ' + totalCharge + ' VNĐ');
                    return false;
                });
                ...
                $('#wrap li').click(function () {
                    ...
                    if (id <= 3)
                        ...
                        targetOffset = single_li_offset + 225;
                    else if (id <= 11)
                        targetOffset = single_li_offset + 225*2;
                    else if (id <= 15)
                        targetOffset = single_li_offset + 225*3;
                   else if (id <= 19)
                        targetOffset = single_li_offset + 225*4;
                    ...
                    <div align="left" style="min-height:800px;">
                        ...
                        <!--    <form action="#" id="cart_form" name="cart_form">
                          -->       <div class="cart-info"> </div>
                                ...
                                <button type="submit" id="Submit" style="color: #90c843">Tính Tiền</button>
                        <!--    </form>
                     -->   </div>
...
$offset = ($curPage - 1) * $rowsPerPage;
            if(isset($_GET["catID"])) {
                $_id = $_GET["catID"];
                $listfull=  Category::loadById($_id);
                $list = Category::loadByIdoffset($_id,$offset,$rowsPerPage);
            } else if(isset($_GET["manuID"])){
                $_id = $_GET["manuID"];
                $listfull= Product::loadById($_id);
                $list = Product::loadByIdoffset($_id,$offset,$rowsPerPage);
            }
            else {
                $listfull=  Product::loadAll();
                $list = Product::loadAlloffset($offset,$rowsPerPage);
            }
            usort($list, "Utils::cmp");
$m = count($listfull);
$n = count($list);
$number_of_pages = ceil($m/ $rowsPerPage);
...
for ($i = 0; $i < $n; $i++) {
    ...
    ?>
                                <li id="<?php echo $i; ?>">
                                    ...
                                    <img  src='images/products/<?php echo $id; ?>/image.jpg'  class="detail_images" alt=""  id="zoom_01"/>
                                ...
                                <div class="detail-view" id="detail-<?php echo $i; ?>">
                                    <div class="close" align="right">
                                        <a href="javascript:void(0)" style="color: #90c843">x</a>
                                    </div>
                                    <img src="images/products/<?php echo $id; ?>/image.jpg" class="items" alt="" width="200px" />
        ...
        if (Context::isLogged()) {
            ...
            ?>         <form method="post" action=""> 
                                            <input class = "text" name="proID" id="proID" type="text" value="<?php echo $id; ?>" style="visibility:hidden; width: 10%" />
                                    <button  type="submit" class="add-to-cart-button" id="btnDatHang" name="btnDatHang" >Đặt Hàng</button>
      </form>
 <?php } ?>                                 
...
<?php } } ?>
            ...
            <div class="content-sidebar">
                    <h4 style="color: #90c843">Các dòng máy</h4>
                    <ul>
<?php
require_once './entities/Category.php';
...
for ($i = 0, $n = count($nsx); $i < $n; $i++) {
    ...
    ?>                       
                    ...
                    <h4 style="color: #90c843">Các hãng sản xuất</h4>
                    <ul>
<?php
            ...
            for($page = 1; $page <= $number_of_pages; $page++)
		{
			if($page == $curPage)
				echo "<a><div class='pagebutton pageactive'>$page</div></a>";
                        ...
                        {
                                if(isset($_GET["page"])){
                                    $page_path="page=".$curPage;
                                    $page_path_next="page=".$page;                      
                                    $path = $_SERVER["REQUEST_URI"];
                                    $new_path=str_replace($page_path, $page_path_next, $path);
                                    echo "<a href='".$new_path."'>"."<div class='pagebutton'>$page</div>"."</a> ";
                                }
                                else{
                                    if(!isset($_GET["manuID"]) && !isset($_GET["catID"])){
                                    echo "<a href='".$_SERVER["REQUEST_URI"]."?page=".$page."'>"."<div class='pagebutton'>$page</div>"."</a> ";}
                                    else echo "<a href='".$_SERVER["REQUEST_URI"]."&page=".$page."'>"."<div class='pagebutton'>$page</div>"."</a> ";
                                }
                        }
		}
            ?>
